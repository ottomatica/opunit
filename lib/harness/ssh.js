...
class SSHConnector {
    ...
    async setup( context, setup ) {
        ...
        let obj = await this._JSSSHExec( cmd, this.sshConfig, 5000, false, {
            setup: setup
        } );
        // format will be PID\nsetup.wait_for\n
        let child = obj.child.split( '\n' );
        if ( child[ 1 ].includes( setup.wait_for ) ) return {
            pid: child[0]
        }
        else {
            console.error( chalk.red( `\t=> ${child[ 1 ]}` ) );
            return;
        }
    }
                        ...
                        stream
                            ...
                            .on('data', function (data) {
                                ...
                                if ( options.setup && data.includes( options.setup.wait_for ) ) {
                                    c.end();
                                    console.log( `\tResolved wait_for condition: Stdout matches "${options.setup.wait_for}"` );
                                    resolve( { child: buffer } );
                                }
                            })

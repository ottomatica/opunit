...
class SSHConnector {
    ...
    async getCPUCores(context) {
        return this.exec(context, 'nproc --all');
    }

    async getMemory(context) {
        return this.exec(context, `grep MemTotal /proc/meminfo | awk '{print $2 / 1024}'`);
    }

    async getDiskSpace(context, diskLocation) {
        return this.exec(context, `df --output=avail -h  ${diskLocation} | grep -P '\\d+.\\d+' -o`);
    }
}

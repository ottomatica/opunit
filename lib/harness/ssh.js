...
class SSHConnector {
    ...
    async isReachable(host, context) {
        ...
        // The pattern below executes the cmd: curl -sL -D - <host_url> -o /dev/null | sed '/^HTTP\/1.1 3[0-9][0-9]/,/^\r$/d' | head -1
        return (await this.exec(context, `curl -sL -D - ${host} -o \/dev\/null | sed \'\/^HTTP\\\/1.1 3[0-9][0-9]\/,\/^\\r$\/d\' | head -1`)).includes('200 OK');
    }

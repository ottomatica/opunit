    ...
    async report(results) {
        for (let resource of Object.keys(results)) {
            if (results[resource].actual) {
                ...
                let message = chalk`{gray [${resource + (results[resource].location || '')}]} {white expected${actualIsBoolean ? '' : ' at least'}:} {gray ${results[resource].expected}${results[resource].unit || ''}} {white actual:} {gray ${results[resource].actual}${results[resource].unit || ''}}`;

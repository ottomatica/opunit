...
class reachableCheck extends Check {
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if(!fileExists && isReachable == "Docker"){
                results.push({
                    resource: path,
                    status: false,
                    msg: "Docker Connector does not support reachable checks for the argument provided",
                });
            } else {
                let status = false;
                if( !fileExists.exists && isReachable ) {
                    // Non-file, but reachable as domain/etc.
                    status = true;
                } else if ( fileExists.exists && fileExists.status && isReachable ) {
                    // File, with right permissions, and is reachable on connector.
                    status = true;
                }
                results.push({
                    resource: path,
    ...
    async _localPathExists(path, expectedPermission, user, group) {
        ...
        path = this._fixHomePathLocal(path);
        let fileExists = await this._fileExists(path);
        ...
        if (fileExists) {
            ...
            // full permission (octal)
                ...
                status = actualPermission === expectedPermission.toString();

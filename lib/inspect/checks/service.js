const Check = require('./check');
const chalk = require('chalk');

class ServiceCheck extends Check {

    constructor(connector, reporter) {
        super(connector, reporter);
    }

    async check(context, args) {
        return await this.getServiceStatus(context, args.name, args.status);
    }

    async getServiceStatus(context, name, expectedStatus) {
        let output = (await this.connector.exec(context, `systemctl status ${name}`));
        let actualStatus = null;
        if (output.includes('Loaded: not-found'))
            actualStatus = 'none';
        else if (output.includes('Active: inactive'))
            actualStatus = 'inactive';
        else if (output.includes('Active: active'))
            actualStatus = 'active';

        return {
            name,
            actualStatus,
            expectedStatus,
            status: actualStatus == expectedStatus
        }
    }

    async report(results) {
        let message = chalk `{gray [${results.name}]} {white expected:} {gray ${results.expectedStatus}} {white actual:} {gray ${results.actualStatus}}`;
        this.reporter.report(message, results.status);
    }
}

// Export factory class
module.exports = ServiceCheck;
